...
Turf and target are seperate in case I want to teleport some distance from a turf the target is not standing on or something.
*/
...
/mob/proc/rename_self(var/role, var/allow_numbers=0)
	spawn(0)
		...
		for(var/i=1,i<=3,i++)	//we get 3 attempts to pick a suitable name.
			newname = input(src,"I am \a [role]. Would I like to change my name to something else?", "Name change",oldname) as text
...
/proc/can_operate(mob/living/carbon/M, mob/living/carbon/user)
	...
	if(locate(/obj/machinery/optable, T))
		. = TRUE
	if(locate(/obj/structure/bed, T) && !locate(/obj/structure/bed/chair, T)) // people like to do executions on chairs
		. = TRUE
