/datum/job
	...
	var/minimal_player_age = 0            // If I have use_age_restriction_for_jobs config option enabled and the database set up, this option will add a requirement for players to be at least minimal_player_age days old. (meaning they first signed in at least that many days before.)
	...
	var/objective						  // the job's goal

	// stat points - setting a stat to 0 keeps it the way it was when human/New() was called
	...
	// variation in stat points, see CLAMPED_RAND
	var/variation = 1
...
/datum/job/proc/equip(var/mob/living/carbon/human/H, var/alt_title, var/datum/mil_branch/branch, var/datum/mil_rank/grade)
	...
	if(!outfit)
		return FALSE
	H.set_points_rand(st=strength, iq=intelligence, dx=dexterity, pr=perception, ran=variation)
...
/datum/job/proc/setup_account(var/mob/living/carbon/human/H)
	...
	if(!(H.species && (H.species.type in economic_species_modifier)))
		return //some bizarre species like shadow, slime, or monkey? I don't get an account.
	...
	if(H.mind)
		...
		remembered_info += "<b>My account funds are:</b> T[M.money]<br>"
	...
	to_chat(H, "<span class='notice'><b>My account number is: [M.account_number], my account pin is: [M.remote_access_pin]</b></span>")
