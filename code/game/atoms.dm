...
// If I want to use this, the atom must have the PROXMOVE flag, and the moving
...
The math involved assumes that the icon is vertical to begin with so unless I want to adjust the math,
...
/atom/attack_hand(mob/user)
	...
	if(climbers.len && !(user in climbers))
		user.visible_message("<span class='warning'>[user.name] shakes \the [src].</span>", \
					"<span class='notice'>I shake \the [src].</span>")
		object_shaken()
...
/atom/proc/can_climb(var/mob/living/user, post_climb_check=0)
	...
	if (!user.Adjacent(src))
		to_chat(user, "<span class='danger'>I can't climb there, the way is blocked.</span>")
		return 0
...
/atom/proc/can_touch(var/mob/user)
	...
	if (user.restrained() || user.buckled)
		to_chat(user, "<span class='notice'>I need my hands and legs free for this.</span>")
		return 0
...
/atom/proc/object_shaken()
	for(var/mob/living/M in climbers)
		M.Weaken(1)
		to_chat(M, "<span class='danger'>I topple as I am shaken off \the [src]!</span>")
		climbers.Cut(1,2)
	...
	for(var/mob/living/M in get_turf(src))
		...
		M.Weaken(3)
		to_chat(M, "<span class='danger'>I topple as \the [src] moves under me!</span>")
		...
		if(prob(25))
			...
			if(!istype(H))
				to_chat(H, "<span class='danger'>I land heavily!</span>")
			...
			if(affecting)
				to_chat(M, "<span class='danger'>I land heavily on my [affecting.name]!</span>")
