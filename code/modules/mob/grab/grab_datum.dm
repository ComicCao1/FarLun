/datum/grab
	...
	var/shield_assailant = 0					// Whether the person you're grabbing will shield I from bullets.,,
	...
	var/success_up = "I upgrade the grab."
	var/success_down = "I downgrade the grab."

	var/fail_up = "I fail to upgrade the grab."
	var/fail_down = "I fail to downgrade the grab."
...
/*
	These procs shouldn't be overriden in the children unless I know what you're doing with them; they handle important core functions.
	Even if I do override them, I should likely be using ..() if I want the behaviour to function properly. That is, of course,
	unless you're writing my own custom handling of things.
*/
...
/datum/grab/proc/hit_with_grab(var/obj/item/grab/G)
	...
	if(G.check_action_cooldown() && !G.attacking)
		...
		to_chat(G.assailant, "<span class='warning'>I must wait before I can do that.</span>")
...
// What happens when I upgrade from one grab state to the next.
...
// What happens when I downgrade from one grab state to the next.
...
// What happens when I let go of someone by either dropping the grab
...
// What happens when I hit the grabbed person with the grab on help intent.
/datum/grab/proc/on_hit_help(var/obj/item/grab/G)
	return 1

// What happens when I hit the grabbed person with the grab on disarm intent.
/datum/grab/proc/on_hit_disarm(var/obj/item/grab/G)
	return 1

// What happens when I hit the grabbed person with the grab on grab intent.
/datum/grab/proc/on_hit_grab(var/obj/item/grab/G)
	return 1

// What happens when I hit the grabbed person with the grab on harm intent.
/datum/grab/proc/on_hit_harm(var/obj/item/grab/G)
	return 1

// What happens when I hit the grabbed person with an open hand and I want it
...
// Used when I want an effect to happen when the grab enters this state as an upgrade
...
/datum/grab/proc/handle_resist(var/obj/item/grab/G)
	...
	if(affecting.incapacitated(INCAPACITATION_KNOCKOUT | INCAPACITATION_STUNNED))
		to_chat(G.assailant, "<span class='warning'>I can't resist in my current state!</span>")
	var/skill_mod = Clamp(affecting.get_skill_difference(SKILL_COMBAT, assailant), -1, 1)
	var/break_strength = breakability + size_difference(affecting, assailant) + skill_mod + ((G.affecting.strength - G.assailant.strength) * 2)
	...
	if(break_strength < 1)
		to_chat(G.assailant, "<span class='warning'>I try to break free but feel that unless something changes, you'll never escape!</span>")
		return
