/mob/living/carbon/human/examine(mob/user)
	...
	if (src.stat)
		...
		if(ishuman(user) && !user.incapacitated() && Adjacent(user))
			spawn(0)
				user.visible_message("<b>\The [user]</b> checks \the [src]'s pulse.", "I check \the [src]'s pulse.")
	...
	if(became_younger)
		msg += "[T.He] looks a lot younger than I remember.\n"
	if(became_older)
		msg += "[T.He] looks a lot older than I remember.\n"
	...
	if(ishuman(user))
		...
		var/strength_difference = UH.strength - strength

		if(!UH.check_pr(9))
			strength_difference += rand(-1, 1)

		switch(strength_difference)
			if(2 to INFINITY)			msg += "<span class='notice'>[T.He] looks much weaker than me.</span>\n"
			if(1)						msg += "<span class='notice'>[T.He] looks weaker than me.</span>\n"
			if(0)						msg += "<span class='notice'>[T.He] looks about as strong as me.</span>\n"
			if(-1)						msg += "<span class='notice'>[T.He] looks stronger than me.</span>\n"
			if(-2 to -INFINITY)			msg += "<span class='notice'>[T.He] looks much stronger than me.</span>\n"

	else
		switch(strength)
			if(0 to 6)			msg += "<span class='notice'>[T.He] looks incredibly weak!</span>\n"
			if(7)				msg += "<span class='notice'>[T.He] looks pretty weak.</span>\n"
			if(8 to 9)			msg += "<span class='notice'>[T.He] looks kind of weak.</span>\n"
			if(10)				msg += "<span class='notice'>[T.He] looks about average strength.</span>\n"
			if(11 to 12)		msg += "<span class='notice'>[T.He] looks kind of strong.</span>\n"
			if(13 to 14)		msg += "<span class='notice'>[T.He] looks pretty strong.</span>\n"
			if(15 to INFINITY)	msg += "<span class='notice'>[T.He] looks incredibly strong!</span>\n"

	switch(decaylevel)
		if(1)		msg += "<span class='notice'>[T.He] is beginning to smell.</span>"
		if(2)		msg += "<span class='notice'>[T.He] is bloated and smells disgusting.</span>"
		if(3)		msg += "<span class='notice'>[T.He] is rotting and blackened, the skin sloughing off. The smell is indescribably foul.</span>"
		if(4)		msg += "<span class='notice'>[T.He] is mostly dessicated now, with only bones remaining of what used to be a person.</span>"
