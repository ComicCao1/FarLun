...
/mob/living/carbon/human/Destroy()
	...
	for(var/organ in organs)
		qdel(organ)
	for(var/mood in happiness_events)
		happiness_events -= mood
		qdel(mood)
	return ..()
...
/mob/living/carbon/human/IsAdvancedToolUser(var/silent)
	if(species.has_fine_manipulation(src))
		return 1
	if(!silent)
		to_chat(src, "<span class='warning'>I don't have the dexterity to use that!</span>")
	return 0
...
/mob/living/carbon/human/proc/play_xylophone()
	if(!src.xylophone)
		visible_message("<span class='warning'>\The [src] begins playing \his ribcage like a xylophone. It's quite spooky.</span>","<span class='notice'>I begin to play a spooky refrain on my ribcage.</span>","<span class='warning'>I hear a spooky xylophone melody.</span>")
...
/mob/living/carbon/human/proc/vomit(var/toxvomit = 0, var/timevomit = 1, var/level = 3)
	...
	if(!lastpuke)
		lastpuke = 1
		to_chat(src, "<span class='warning'>I feel nauseous...</span>")
		if(level > 1)
			sleep(150 / timevomit)	//15 seconds until second warning
			to_chat(src, "<span class='warning'>I feel like I'm about to throw up!</span>")
			if(level > 2)
				sleep(100 / timevomit)	//and I have 10 more for mad dash to the bucket
				Stun(3)
				if(nutrition < 40)
...
/mob/living/carbon/human/proc/remotesay()
	...
	for(var/mob/living/carbon/h in world)
		creatures += h
	var/mob/target = input("Who do I want to project my mind to ?") as null|anything in creatures
	if (isnull(target))
		return

	var/say = sanitize(input("What do I wish to say"))
	if(mRemotetalk in target.mutations)
		target.show_message("<span class='notice'>I hear [src.real_name]'s voice: [say]</span>")
	else
		target.show_message("<span class='notice'>I hear a voice that seems to echo around the room: [say]</span>")
	usr.show_message("<span class='notice'>I project my mind into [target.real_name]: [say]</span>")
...
/mob/living/carbon/human/proc/remoteobserve()
	...
	var/mob/target = input ("Who do I want to project my mind to ?") as mob in creatures
...
/mob/living/carbon/human/proc/handle_embedded_and_stomach_objects()
	for(var/obj/item/organ/external/organ in src.organs)
		if(organ.splinted)
			continue
		for(var/obj/item/O in organ.implants)
			if(!istype(O,/obj/item/weapon/implant) && O.w_class > 1 && prob(5)) //Moving with things stuck in I could be bad.
		...
		for(var/obj/item/O in stomach_contents)
			if(O.edge || O.sharp)
				if(prob(1))
					...
					if(can_feel_pain())
						to_chat(src, "<span class='danger'>I feel something rip out of my stomach!</span>")
						groin.embed(O)
				else if(prob(5))
...
/mob/living/carbon/human/proc/jossle_internal_object(var/obj/item/organ/external/organ, var/obj/item/O)
	...
	if(!can_feel_pain())
		to_chat(src, "<span class='warning'>I feel [O] moving inside my [organ.name].</span>")
	else
		var/msg = pick( \
			"<span class='warning'>A spike of pain jolts my [organ.name] as I bump [O] inside.</span>", \
			"<span class='warning'>My movement jostles [O] in my [organ.name] painfully.</span>", \
			"<span class='warning'>My movement jostles [O] in my [organ.name] painfully.</span>")
...
/mob/living/carbon/human/verb/check_pulse()
	...
	set desc = "Approximately count somebody's pulse. Requires me to stand still at least 6 seconds."
	set src in view(1)
	var/self = 0
	...
	if(!self)
		...
		"I begin counting [src]'s pulse")
	else
		usr.visible_message("<span class='notice'>[usr] begins counting their pulse.</span>",\
		"I begin counting my pulse.")
	...
	to_chat(usr, "[self ? "I must" : "We both must"] remain still until counting is finished.")
	if(do_mob(usr, src, 60))
		var/message = "<span class='notice'>[self ? "My" : "[src]'s"] pulse is [src.get_pulse(GETPULSE_HAND)].</span>"
		to_chat(usr, message)
	else
		to_chat(usr, "<span class='warning'>I failed to check the pulse. Try again.</span>")

/mob/living/carbon/human/RightClick(mob/user)
	if(!user.get_active_hand() && src in view(1,user))
		if(user.a_intent == I_HELP)
			check_pulse(user)
		else if(user.a_intent == I_GRAB && /mob/proc/yank_out_object in verbs)
			yank_out_object(user)
		

/mob/living/carbon/human/verb/lookup()
	set name = "Look up"
	set desc = "If I want to know what's above."
	set category = "IC"
	...
	if(!is_physically_disabled())
		...
		if(shadow)
			...
			if(istype(above, /turf/simulated/open))
				to_chat(src, "<span class='notice'>I look up.</span>")
				if(client)
					reset_view(shadow)
				return
		if(above)
			to_chat(src, "<span class='notice'>I can see \the [above].</span>")
		else
			to_chat(src, "<span class='notice'>I can't see anything.</span>")
	else
		to_chat(src, "<span class='notice'>I can't look up right now.</span>")
	return
...
/mob/living/carbon/human/proc/undislocate()
	...
	if(usr.stat > 0)
		to_chat(usr, "I'm unconcious and cannot do that!")
		return

	if(usr.restrained())
		to_chat(usr, "I'm restrained and cannot do that!")
		return
	...
	var/self = null
	if(S == U)
		self = 1 // Removing object from myself.

	var/list/limbs = list()
	for(var/limb in organs_by_name)
		var/obj/item/organ/external/current_limb = organs_by_name[limb]
		if(current_limb && current_limb.dislocated > 0 && !current_limb.is_parent_dislocated()) //if the parent is also dislocated I will have to relocate that first
			limbs |= current_limb
	var/obj/item/organ/external/current_limb = input(usr,"Which joint do I wish to relocate?") as null|anything in limbs

	if(!current_limb)
		return

	if(self)
		to_chat(src, "<span class='warning'>I brace myself to relocate my [current_limb.joint]...</span>")
	else
		to_chat(U, "<span class='warning'>I begin to relocate [S]'s [current_limb.joint]...</span>")
	...
	if(self)
		to_chat(src, "<span class='danger'>I pop my [current_limb.joint] back in!</span>")
	else
		to_chat(U, "<span class='danger'>I pop [S]'s [current_limb.joint] back in!</span>")
		to_chat(S, "<span class='danger'>[U] pops my [current_limb.joint] back in!</span>")
...
/mob/living/carbon/human/can_stand_overridden()
	if(wearing_rig && wearing_rig.ai_can_move_suit(check_for_ai = 1))
		// Actually missing a leg will screw I up. Everything else can be compensated for.
...
/mob/living/carbon/human/proc/get_pulse(var/method)	//method 0 is for hands, 1 is for machines, more accurate
	...
	if(!H)
		return "0"
	if(H.open && !method)
		return "muddled and unclear; I can't seem to find a vein"
...
/mob/living/carbon/human/help_shake_act(mob/living/carbon/M)
	if(src != M)
		...
		var/msg = "Let's see how I'm doing.\n"
		visible_message( \
		...
		for(var/obj/item/organ/external/org in organs)
			...
			if(status.len)
				msg += "<span class='organHealth'>My [org.name] is <span class='organBad'>[english_list(status)]</span>.</span>\n"
			else
				msg += "<span class='organHealth'>My [org.name] is <span class='notice'>OK</span>.</span>\n"
		to_chat(src, msg)
...
//Point at which I dun breathe no more. Separate from asystole crit, which is heart-related.
...
/mob/living/carbon/human/proc/update_buried(new_val, force=0)
	if(buried != 2 || force)
		buried = new_val