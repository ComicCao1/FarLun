...
#define HEAT_DAMAGE_LEVEL_3 8 //Amount of damage applied when my body temperature passes the 1000K point

#define COLD_DAMAGE_LEVEL_1 0.5 //Amount of damage applied when my body temperature just passes the 260.15k safety point
#define COLD_DAMAGE_LEVEL_2 1.5 //Amount of damage applied when my body temperature passes the 200K point
#define COLD_DAMAGE_LEVEL_3 3 //Amount of damage applied when my body temperature passes the 120K point
...
/mob/living/carbon/human/proc/get_pressure_weakness()
	...
	if(wear_suit && (wear_suit.item_flags & ITEM_FLAG_STOPPRESSUREDAMAGE) && head && (head.item_flags & ITEM_FLAG_STOPPRESSUREDAMAGE)) // Complete set of pressure-proof suit worn, assume fully sealed.
		...
		// Handles breaches in my space suit. 10 suit damage equals a 100% loss of pressure protection.
...
/mob/living/carbon/human/proc/handle_loneliness() // fucking stick together holy shit
	var/boost = 0
	
	for(var/mob/living/carbon/human/H in oview(7, src))
		if(H.stat || H == src)  // no unconscious or dead people
			continue
		boost += 1
	
	if(!boost)
		boost -= 1
		if(!check_iq(10)) // dumb people get half as lonely
			boost /= 2

	loneliness += boost

	loneliness = Clamp(loneliness, 0, 550)

	switch(loneliness)
		if(0 to 150)
			add_happiness_event("loneliness", "I'm incredibly lonely!", -400, 0)
		if(151 to 250)
			add_happiness_event("loneliness", "I'm lonely.", -200, 0)
		if(251 to 350)
			remove_happiness_event("loneliness") // dont really care
		if(351 to 450)
			add_happiness_event("loneliness", "I like being near others.", 100, 0)
		if(451 to 550)
			add_happiness_event("loneliness", "I love being near others!", 200, 0)


/mob/living/carbon/human/proc/handle_decay() // thanks paradise
	...
	if(decaytime <= 4 MINUTES) //stinky, flies
		return

	if(decaytime > 4 MINUTES && decaytime <= 8 MINUTES) //decaylevel2 -- bloated and very stinky
		start_flies()
		decaylevel = 1

	if(decaytime > 12 MINUTES && decaytime <= 16 MINUTES)//decaylevel3 -- rotting and gross
		decaylevel = 2

	if(decaytime > 20 MINUTES && decaytime <= 30 MINUTES)//decaylevel4 -- skeleton
		decaylevel = 3
	...
	for(var/mob/living/carbon/human/H in range(decaylevel, src))
		if(H.mind && H.mind.assigned_role == "Coroner")
			continue // this would keep them from doing their job

		var/mood_hit = 0// no one likes seeing dead bodies, except maybe coroner
		mood_hit -= ((decaylevel*10)+(700-(get_dist(src, H)*100)))
		if(mood_hit < 0)
			if(H.happiness_events["Seen dead body [real_name]"])
				var/datum/happiness_event/h = H.happiness_events["Seen dead body [real_name]"]
				if(h.value <= mood_hit)
					continue
			H.add_happiness_event("Seen dead body [real_name]", "I've seen [src.name == "Unknown" ? "someone" : "[src]" ]'s disgusting, rotted dead body!", mood_hit, 3 MINUTES)
	...
	if(!buried && decaylevel >= 3 && prob(1))
		for(var/mob/living/carbon/human/H in oview(7, src))
			if(!H.stat)
				return
	
		var/turf/simulated/floor/T = pick(range(3, src))
		new /mob/living/simple_animal/hostile/mutants/cursed_child(T)

/mob/living/carbon/human/proc/start_flies()
	if(flies)
		return
	flies = 1
	overlays += image(icon='icons/effects/rotting.dmi', icon_state="rotten")

/mob/living/carbon/human/proc/end_flies()
	if(!flies)
		return
	flies = 0
	overlays -= image(icon='icons/effects/rotting.dmi', icon_state="rotten")
	...
	if(!radiation)
			...
			if(!isSynthetic())
				...
				if(prob(5) && prob(100 * RADIATION_SPEED_COEFFICIENT) && species.get_bodytype(src) == SPECIES_HUMAN) //apes go bald
					if((h_style != "Bald" || f_style != "Shaved" ))
						to_chat(src, "<span class='warning'>my hair falls out.</span>")
						h_style = "Bald"
						f_style = "Shaved"
						update_hair()
		...
		if (radiation > 75)
			...
			if(!isSynthetic())
				...
				if(prob(1))
					to_chat(src, "<span class='warning'>I feel strange!</span>")
...
/mob/living/carbon/human/handle_environment(datum/gas_mixture/environment)
	...
	if(relative_density > 0.02) //don't bother if we are in vacuum or near-vacuum
		...
		//Body temperature adjusts depending on surrounding atmosphere based on my thermal protection (convection)
		var/temp_adj = 0
		if(loc_temp < bodytemperature)			//Place is colder than we are
			var/thermal_protection = get_cold_protection(loc_temp) //This returns a 0 - 1 value, which corresponds to the percentage of protection based on what I'm wearing and what I'm exposed to.
			if(thermal_protection < 1)
				temp_adj = (1-thermal_protection) * ((loc_temp - bodytemperature) / BODYTEMP_COLD_DIVISOR)	//this will be negative
		else if (loc_temp > bodytemperature)			//Place is hotter than we are
			var/thermal_protection = get_heat_protection(loc_temp) //This returns a 0 - 1 value, which corresponds to the percentage of protection based on what I'm wearing and what I'm exposed to.
			if(thermal_protection < 1)
...
/mob/living/carbon/human/proc/stabilize_body_temperature()
	...
	if(bodytemperature < species.cold_level_1) //260.15 is 310.15 - 50, the temperature where I start to feel effects.
...
//		log_debug("Norm. Difference = [body_temperature_difference]. Recovering [recovery_amt]")
		bodytemperature += recovery_amt
	else if(bodytemperature > species.heat_level_1) //360.15 is 310.15 + 50, the temperature where I start to feel effects.
		//We totally need a sweat system cause it totally makes sense...~
		var/recovery_amt = min((body_temperature_difference / BODYTEMP_AUTORECOVERY_DIVISOR), -BODYTEMP_AUTORECOVERY_MINIMUM)	//We're dealing with negative numbers
//		log_debug("Hot. Difference = [body_temperature_difference]. Recovering [recovery_amt]")
		bodytemperature += recovery_amt

	//This proc returns a number made up of the flags for body parts which I'm protected on. (such as HEAD, UPPER_TORSO, LOWER_TORSO, etc. See setup.dm for the full list)
/mob/living/carbon/human/proc/get_heat_protection_flags(temperature) //Temperature is the temperature I'm being exposed to.
	. = 0
...
/mob/living/carbon/human/get_heat_protection(temperature) //Temperature is the temperature I'm being exposed to.
...
/mob/living/carbon/human/handle_regular_status_updates()
	...
	if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if (drowsyness > 0)
			...
			if(drowsyness > 10)
				...
				if((prob(zzzchance) || drowsyness >= 60))
					if(stat == CONSCIOUS)
						to_chat(src, "<span class='notice'>I'm about to fall asleep...</span>")
					Sleeping(5)

		// If I'm dirty, youmy ves will become dirty, too.
		...
		if(stasis_value > 1 && drowsyness < stasis_value * 4)
			...
			if(!stat && prob(1))
				to_chat(src, "<span class='notice'>I feel slow and sluggish...</span>")
		if(happiness < 200)
			jumpscare()
	return 1
...
/mob/living/carbon/human/proc/update_happiness_icon()
	if(happiness_icon)
		switch(happiness)
			if(800 to INFINITY)
				happiness_icon.icon_state = "happy2"
				happiness_text = "Great"
				happiness_damage = 1.5
			if(600 to 800)
				happiness_icon.icon_state = "happy1"
				happiness_text = "Good"
				happiness_damage = 1.25
			if(400 to 600)
				happiness_icon.icon_state = "neutral"
				happiness_text = "Indifferent"
				happiness_damage = 1
			if(200 to 400)					
				happiness_icon.icon_state = "sad1"
				happiness_text = "Bad"
				happiness_damage = 0.75
			else
				happiness_icon.icon_state = "sad2"
				happiness_text = "Terrible"
				happiness_damage = 0.5
...
/mob/living/carbon/human/proc/handle_shock()
	...
	if(shock_stage == 10)
		...
		// which triggers this proc, which calls custom_pain(), etc. Make sure I call it with nohalloss = TRUE in these cases!
		custom_pain("[pick("It hurts so much", "I really need some painkillers", "Dear god, the pain")]!", 10, nohalloss = TRUE)
	...
	if(shock_stage == 40)
		custom_pain("[pick("The pain is excruciating", "Please, just end the pain", "my whole body is going numb")]!", 40, nohalloss = TRUE)
	if (shock_stage >= 60)
		if(shock_stage == 60) visible_message("<b>[src]</b>'s body becomes limp.")
		if (prob(2))
			custom_pain("[pick("The pain is excruciating", "Please, just end the pain", "my whole body is going numb")]!", shock_stage, nohalloss = TRUE)
			Weaken(10)
	...
	if(shock_stage >= 120)
		if (prob(2))
			custom_pain("[pick("I black out", "I feel like I could die any moment now", "I'm about to lose consciousness")]!", shock_stage, nohalloss = TRUE)
			Paralyse(5)
...
/*********************************************************************************
							MOOD PROCS BY CthulhuOnIce
*********************************************************************************/


/mob/living/carbon/human/proc/add_happiness_event(category, message, weight, timeout, silent_override=0)
	if(stat == DEAD)		return  // dead men get no moods

	var/datum/happiness_event/happiness_event = new /datum/happiness_event
	var/old_happiness = happiness

	happiness_event.title = category
	happiness_event.msg = message
	happiness_event.value = weight
	
	if(happiness_events[category])
		remove_happiness_event(category, silent=TRUE)

	happiness_events[category] = happiness_event 
	happiness += weight
	
	if(happiness > old_happiness)			to_chat(src, "<span class='moodGood'>My mood improves.</span>")
	else if(happiness < old_happiness)		to_chat(src, "<span class='moodBad'>My mood worsens.</span>")
	
	update_happiness_icon()

	if(timeout)
		spawn(timeout)
			remove_happiness_event(category)

/mob/living/carbon/human/proc/remove_happiness_event(category, silent=FALSE)
	if(stat == DEAD)		return
	
	var/old_happiness = happiness

	if(happiness_events[category])
		var/datum/happiness_event/h = happiness_events[category]
		happiness -= h.value
		if(!silent)
			if(happiness < old_happiness)				to_chat(src, "<span class='moodBad'>My mood worsens.</span>")
			else if(happiness > old_happiness)			to_chat(src, "<span class='moodGood'>My mood improves.</span>")

		update_happiness_icon()

		happiness_events -= category
		qdel(h)

/mob/living/carbon/human/proc/list_moodlets()  // called when I click the mood icon, prints out all my moodlets
	var/msg

	if(!happiness_events.len)
		msg = "I don't feel any particular way."
		to_chat(src, msg)
		return
	
	msg += "Let's See How I'm Feeling\n"
	for(var/m in happiness_events)
		var/datum/happiness_event/h = happiness_events[m]
		if(h)
			if(h.value < 0)
				msg += "- \t<span class='moodBad'>[h.msg]</span>\n"
			else
				msg += "- \t<span class='moodGood'>[h.msg]</span>\n"
	msg += "Overall, I Feel <b>[happiness_text].</b>"
	to_chat(src, msg)
	return

/mob/living/carbon/human/proc/clearmoods()  // proc triggered when I receive amnesia-causing brain damage
	for(var/m in happiness_events)
		var/datum/happiness_event/h = happiness_events[m]
		if(h)
			remove_happiness_event(h.title, silent=TRUE)

//	TODO: Debug
/mob/living/carbon/human/proc/jumpscare()
	if(prob(5))
		var/scare = pick('sound/effects/badmood1.ogg','sound/effects/badmood2.ogg','sound/effects/badmood3.ogg','sound/effects/badmood4.ogg')
		sound_to(src, scare)	
		if(prob(30))
			overlay_fullscreen("jumpscare", /obj/screen/fullscreen/screamer, rand(1,13))
			spawn(10)
				clear_fullscreen("jumpscare", 7)
			drop_l_hand()
			drop_r_hand()
