...
/mob/living/carbon/human/attack_hand(mob/living/carbon/M as mob)
	...
	if(istype(H))
		...
		if(!temp || !temp.is_usable())
			to_chat(H, "<span class='warning'>I can't use my hand.</span>")
			return
	...
	switch(M.a_intent)
			...
			if(istype(H) && (is_asystole() || (status_flags & FAKEDEATH)))
				...
				if(stat < DEAD)
					...
					if(!H.check_has_mouth())
						to_chat(H, "<span class='warning'>I don't have a mouth, I cannot do mouth-to-mouth resustication!</span>")
						return
					if(!check_has_mouth())
						to_chat(H, "<span class='warning'>They don't have a mouth, I cannot do mouth-to-mouth resustication!</span>")
						return
					if((H.head && (H.head.body_parts_covered & FACE)) || (H.wear_mask && (H.wear_mask.body_parts_covered & FACE)))
						to_chat(H, "<span class='warning'>I need to remove my mouth covering for mouth-to-mouth resustication!</span>")
						return 0
					if((head && (head.body_parts_covered & FACE)) || (wear_mask && (wear_mask.body_parts_covered & FACE)))
						to_chat(H, "<span class='warning'>I need to remove \the [src]'s mouth covering for mouth-to-mouth resustication!</span>")
						return 0
					if (!H.internal_organs_by_name[H.species.breathing_organ])
						to_chat(H, "<span class='danger'>I need lungs for mouth-to-mouth resustication!</span>")
						return
					if(!need_breathe())
						return
					...
					if(L)
						...
						if(!fail)
							to_chat(src, "<span class='notice'>I feel a breath of fresh air enter my lungs. It feels good.</span>")
		...
		if(I_HURT)
			...
			if(world.time < H.last_attack + attack.delay)
				to_chat(H, "<span class='notice'>I can't attack again so soon.</span>")
				return 0
			else
			...
			if(!accurate)
				/* ~Hubblenaut
					...
					If aiming for chest:
						27.3% chance I hit my target organ
						70.5% chance I hit a random other organ
						 2.2% chance I miss

					If aiming for something else:
						23.2% chance I hit my target organ
						56.8% chance I hit a random other organ
						15.0% chance I miss
			...
			real_damage *= H.happiness_damage
			rand_damage *= H.happiness_damage

			if(istype(H))
				real_damage *= (H.strength / strength)
			else
				real_damage *= (H.strength / 10)
...
/*
	...
	If I am applying pressure to myself and attempt to grab someone else, you'll change what I am holding in my active hand which will stop do_mob()
	If I am applying pressure to another and attempt to apply pressure to myself, you'll have to switch to an empty hand which will also stop do_mob()
	Changing targeted zones should also stop do_mob(), preventing I from applying pressure to more than one body part at once.
*/
/mob/living/carbon/human/proc/apply_pressure(mob/living/user, var/target_zone)
	...
	if(organ.applied_pressure)
		var/message = "<span class='warning'>[ismob(organ.applied_pressure)? "Someone" : "\A [organ.applied_pressure]"] is already applying pressure to [user == src? "My [organ.name]" : "[src]'s [organ.name]"].</span>"
		to_chat(user, message)
		return 0

	if(user == src)
		user.visible_message("\The [user] starts applying pressure to \his [organ.name]!", "I start applying pressure to my [organ.name]!")
	else
		user.visible_message("\The [user] starts applying pressure to [src]'s [organ.name]!", "I start applying pressure to [src]'s [organ.name]!")
	spawn(0)
		...
		if(user == src)
			user.visible_message("\The [user] stops applying pressure to \his [organ.name]!", "I stop applying pressure to my [organ.name]!")
		else
			user.visible_message("\The [user] stops applying pressure to [src]'s [organ.name]!", "I stop applying pressure to [src]'s [organ.name]!")
