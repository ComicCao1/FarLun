...
/datum/species
    ...
    var/halloss_message_self = "The pain is too severe for I to keep going..."
    ...
    var/list/heat_discomfort_strings = list(
        "I feel sweat drip down my neck.",
    ...
    var/list/cold_discomfort_strings = list(
        "I feel chilly.",
        "I shiver suddenly.",
        "My chilly flesh stands out in goosebumps."
        )
...
The slots that I can use are found in items_clothing.dm and are the inventory slot string ones, so make sure
    I use the _str version of the slot.
*/
...
/datum/species/proc/hug(var/mob/living/carbon/human/H,var/mob/living/target)
    ...
    H.visible_message("<span class='notice'>[H] hugs [target] to make [t_him] feel better!</span>", \
                    "<span class='notice'>I hug [target] to make [t_him] feel better!</span>")
	
    if(ishuman(target))
        var/mob/living/carbon/human/T = target
        if(T.stat < DEAD)
            H.add_happiness_event("[T]-hug-given", "Giving hugs is nice.", 50, 3000)  // makes them all warm n' fuzzy
            T.add_happiness_event("[H]-hug-received", "Getting a hug is nice.", 65, 3000) // getting hugs feels nicer
        else
            H.add_happiness_event("[T]-hug-given", "I hugged [T]'s body! Gross!", -100, 3000)
