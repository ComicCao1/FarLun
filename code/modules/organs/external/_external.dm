...
/obj/item/organ/external
	...
	var/dislocated = 0                 // If I target a joint, I can dislocate the limb, causing temporary damage to the organ.
...
/obj/item/organ/external/proc/robo_repair(var/repair_amount, var/damage_type, var/damage_desc, obj/item/tool, mob/living/user)
	...
	if(user == src.owner)
		...
		if(grasp)
			to_chat(user, "<span class='warning'>I can't reach my [src.name] while holding [tool] in my [owner.get_bodypart_name(grasp)].</span>")
			return 0

	user.setClickCooldown(DEFAULT_ATTACK_COOLDOWN)
	if(!do_mob(user, owner, 10))
		to_chat(user, "<span class='warning'>I must stand still to do that.</span>")
		return 0
...
/obj/item/organ/external/proc/createwound(var/type = CUT, var/damage, var/surgical)
	...
	//moved these before the open_wound check so that having many small wounds for example doesn't somehow protect I from taking internal damage (because of the return)
	...
	if(!surgical && (type in list(CUT, PIERCE, BRUISE)) && damage > 15 && local_damage > 30)
		...
		if(prob(ceil(damage/4)) && sever_tendon())
			internal_damage = TRUE
		if(internal_damage)
			owner.custom_pain("I feel something rip in my [name]!", 50, affecting = src)
			...
			if(compatible_wounds.len)
				...
				if(prob(25))
					if(BP_IS_CRYSTAL(src))
						...
						"<span class='danger'>The cracks in my [name] spread.</span>",\
						"<span class='danger'>I hear the cracking of crystal.</span>")
					else if(BP_IS_ROBOTIC(src))
						owner.visible_message("<span class='danger'>The damage to \the [owner]'s [name] worsens.</span>",\
						"<span class='danger'>The damage to my [name] worsens.</span>",\
						"<span class='danger'>I hear the screech of abused metal.</span>")
					else
						owner.visible_message("<span class='danger'>The wound on \the [owner]'s [name] widens with a nasty ripping noise.</span>",\
						"<span class='danger'>The wound on my [name] widens with a nasty ripping noise.</span>",\
						"<span class='danger'>I hear a nasty ripping noise, as if flesh is being torn apart.</span>")
				return W
...
infection levels. Since infection growth is exponential, I can adjust the time it takes to get
...
INFECTION_LEVEL_THREE	above this germ level the player will take additional toxin damage per second, and will die in minutes without
						antitox. also, above this germ level I will need to overdose on spaceacillin and get rest to reduce the germ_level.
...
/obj/item/organ/external/handle_germ_effects()
	...
	if(germ_level >= INFECTION_LEVEL_THREE && antibiotics < REAGENTS_OVERDOSE)	//overdosing is necessary to stop severe infections
		if (!(status & ORGAN_DEAD))
			status |= ORGAN_DEAD
			to_chat(owner, "<span class='notice'>I can't feel my [name] anymore...</span>")
			owner.update_body(1)
...
/obj/item/organ/external/proc/get_droplimb_messages_for(var/droptype, var/clean)
	...
	if(BP_IS_CRYSTAL(src))
		...
		return list(
			...
			"My [src.name] shatters into a thousand pieces!",
			"I hear the sound of something shattering!"
		)
	else
		switch(droptype)
			if(DROPLIMB_EDGE)
				if(!clean)
					...
					return list(
						...
						"My [src.name] goes flying off!",
						"I hear a terrible sound of [gore_sound]."
						)
			if(DROPLIMB_BURN)
				var/gore = "[BP_IS_ROBOTIC(src) ? "": " of burning flesh"]"
				return list(
					"\The [owner]'s [src.name] flashes away into ashes!",
					"My [src.name] flashes away into ashes!",
					"I hear a crackling sound[gore]."
					)
			if(DROPLIMB_BLUNT)
				...
				return list(
					...
					"My [src.name] explodes[gore]!",
					"I hear the [gore_sound]."
					)
...
/obj/item/organ/external/proc/clampe()
	var/rval = 0
...
/obj/item/organ/external/proc/fracture()
	...
	if(owner)
		owner.visible_message(\
			"<span class='danger'>I hear a loud cracking sound coming from \the [owner].</span>",\
			"<span class='danger'>Something feels like it shattered in my [name]!</span>",\
	...
	// Fractures have a chance of getting I out of restraints
	if (prob(25))
		release_restraints()
...
/obj/item/organ/external/removed(var/mob/living/user, var/ignore_children = 0)
	...
	if(is_robotic && (status & ORGAN_SABOTAGED))
		victim.visible_message(
			...
			"<span class='danger'>my [src.name] explodes!</span>",\
			"<span class='danger'>I hear an explosion!</span>")
...
/obj/item/organ/external/proc/disfigure(var/type = "brute")
	...
	if(owner)
		if(type == "brute")
			owner.visible_message("<span class='danger'>I hear a sickening cracking sound coming from \the [owner]'s [name].</span>",	\
			"<span class='danger'>my [name] becomes a mangled mess!</span>",	\
			"<span class='danger'>I hear a sickening crack.</span>")
		else
			owner.visible_message("<span class='danger'>\The [owner]'s [name] melts away, turning into mangled mess!</span>",	\
			"<span class='danger'>my [name] melts away!</span>",	\
			"<span class='danger'>I hear a sickening sizzle.</span>")
...
/obj/item/organ/external/proc/jostle_bone(force)
	...
	if(internal_organs.len && prob(brute_dam + force))
		owner.custom_pain("A piece of bone in my [encased ? encased : name] moves painfully!", 50, affecting = src)
...
/obj/item/organ/external/proc/inspect(mob/user)
	...
	if(wounds.len)
		to_chat(user, "<span class='warning'>I find [get_wounds_desc()]</span>")
		var/list/stuff = list()
		for(var/datum/wound/wound in wounds)
		...
		to_chat(user, "<span class='notice'>I find no visible wounds.</span>")

	to_chat(user, "<span class='notice'>Checking skin now...</span>")
	if(!do_mob(user, owner, 10))
		to_chat(user, "<span class='notice'>I must stand still to check [owner]'s skin for abnormalities.</span>")
		return
	...
	if(!do_mob(user, owner, 10))
		to_chat(user, "<span class='notice'>I must stand still to feel [src] for fractures.</span>")
		return

	if(status & ORGAN_BROKEN)
		to_chat(user, "<span class='warning'>The [encased ? encased : "bone in the [name]"] moves slightly when I poke it!</span>")
		owner.custom_pain("My [name] hurts where it's poked.",40, affecting = src)
	else
...
/obj/item/organ/external/proc/jointlock(mob/attacker)
	...
	if(armor < 100)
		to_chat(owner, "<span class='danger'>I feel extreme pain!</span>")
