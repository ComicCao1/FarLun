...
/datum/surgery_step/proc/success_chance(mob/living/user, mob/living/carbon/human/target, obj/item/tool)
	...
	if(ishuman(user))
		...
		if(H.eye_blind)
			. -= 60
		if(dexterity_requirement)
			. += (H.dexterity - dexterity_requirement)*10
...
/obj/item/proc/do_surgery(mob/living/carbon/M, mob/living/user, fuckup_prob)
	...
	if(zone in M.op_stage.in_progress) //Can't operate on someone repeatedly.
		to_chat(user, "<span class='warning'>I can't operate on this area while surgery is already in progress.</span>")
		return 1
		...
		if(S.tool_quality(src))
			...
			if(step_is_valid && S.is_valid_target(M))
				...
				if(prob(S.success_chance(user, M, src)) &&  do_mob(user, M, duration))
					...
					to_chat(user, "<span class='warning'>I must remain close to my patient to conduct surgery.</span>")
				if (M)
