...
/datum/surgery_step/generic/cut_with_laser/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start the bloodless incision on [target]'s [affected.name] with \the [tool].")
	target.custom_pain("I feel a horrible, searing pain in my [affected.name]!",50, affecting = affected)
	..()
...
/datum/surgery_step/generic/cut_with_laser/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have made a bloodless incision on [target]'s [affected.name] with \the [tool].</span>")
...
/datum/surgery_step/generic/cut_with_laser/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips as the blade sputters, searing a long gash in [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/generic/incision_manager/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start to construct a prepared incision on and within [target]'s [affected.name] with \the [tool].")
	target.custom_pain("I feel a horrible, searing pain in my [affected.name] as it is pushed apart!",50, affecting = affected)
	..()
...
/datum/surgery_step/generic/incision_manager/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have constructed a prepared incision on and within [target]'s [affected.name] with \the [tool].</span>")

	affected.createwound(CUT, affected.min_broken_damage/2, 1) // incision
	affected.clampe() // clamp
...
/datum/surgery_step/generic/incision_manager/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand jolts as the system sparks, ripping a gruesome hole in [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/generic/cut_open/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start the incision on [target]'s [affected.name] with \the [tool].")
	target.custom_pain("I feel a horrible pain as if from a sharp knife in my [affected.name]!",40, affecting = affected)
	..()
...
/datum/surgery_step/generic/cut_open/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have made an incision on [target]'s [affected.name] with \the [tool].</span>")
	affected.createwound(CUT, affected.min_broken_damage/2, 1)
...
/datum/surgery_step/generic/cut_open/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, slicing open [target]'s [affected.name] in the wrong place with \the [tool]!</span>")
...
/datum/surgery_step/generic/clamp_bleeders/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start clamping bleeders in [target]'s [affected.name] with \the [tool].")
	target.custom_pain("The pain in my [affected.name] is maddening!",40, affecting = affected)
	..()
...
/datum/surgery_step/generic/clamp_bleeders/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I clamp bleeders in [target]'s [affected.name] with \the [tool].</span>")
	affected.clampe()
	spread_germs_to_organ(affected, user)
...
/datum/surgery_step/generic/clamp_bleeders/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, tearing blood vessels and causing massive bleeding in [target]'s [affected.name] with \the [tool]!</span>")
	affected.take_external_damage(10, 0, (DAM_SHARP|DAM_EDGE), used_weapon = tool)
...
/datum/surgery_step/generic/retract_skin/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start to pry open the incision on [target]'s [affected.name] with \the [tool].")
	target.custom_pain("It feels like the skin on my [affected.name] is on fire!",40,affecting = affected)
	..()
...
/datum/surgery_step/generic/retract_skin/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I keep the incision open on [target]'s [affected.name] with \the [tool].</span>")
...
/datum/surgery_step/generic/retract_skin/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, tearing the edges of the incision on [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/generic/cauterize/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I am beginning to cauterize[W ? " \a [W.desc] on" : ""] \the [target]'s [affected.name] with \the [tool].")
	target.custom_pain("My [affected.name] is being burned!",40,affecting = affected)
	..()
...
/datum/surgery_step/generic/cauterize/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I cauterize[W ? " \a [W.desc] on" : ""] \the [target]'s [affected.name] with \the [tool].</span>")
	if(W)
		W.close()
	if(affected.is_stump())
...
/datum/surgery_step/generic/cauterize/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, leaving a small burn on [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/generic/amputate/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (affected == null)
		return 0
	if (affected.how_open())
		to_chat(user,"<span class='warning'>I can't get a clean cut with incisions getting in the way.</span>")
		return SURGERY_FAILURE
...
/datum/surgery_step/generic/amputate/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<FONT size=3>I am beginning to cut through [target]'s [affected.amputation_point] with \the [tool].</FONT>")
	target.custom_pain("My [affected.amputation_point] is being ripped apart!",100,affecting = affected)
	..()
...
/datum/surgery_step/generic/amputate/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I amputate [target]'s [affected.name] with \the [tool].</span>")
...
/datum/surgery_step/generic/amputate/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, sawwing through the bone in [target]'s [affected.name] with \the [tool]!</span>")
